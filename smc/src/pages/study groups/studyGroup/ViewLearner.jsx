    ...
    const fetchUser = async () => {
      try {
        const response = await axios.get(
          `${API}/api/getusersbyidchat?id=${userId}`
        );
    ...
    <div className="font-poppins ">
      {User && (
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className=" lg:col-span-8 md:col-span-7 col-span-12 bg-darkgray   rounded-4xl lg:h-[870px] md:h-[700px] h-[600px] p-4">
            <p className="text-center text-white text-2xl my-4">
              Learner Profile
            </p>

            <div className="text-white px-2 mt-5">
              <p className=" text-lg font-semibold">Learning Goals : </p>
              <p className="text-base font-light my-1">{User.goals || "No Learner Goals Found"}</p>
            </div>

            <div className="text-white px-2 mt-3">
              <p className=" text-lg font-semibold">Experience Level  : </p>
              <p className="text-base font-light my-1">{User.experience || "No Experience Found"}</p>
            </div>

            <div className="text-white px-2 mt-3">
              <p className=" text-lg font-semibold">Area Of Interest  : </p>
              <p className="text-base font-light my-1">{User.areaOfInterest || "No AreaOfInterest Found"}</p>
            </div>

            <div className="text-white px-2 mt-3">
              <p className=" text-lg font-semibold">Resource Needs  : </p>
              <p className="text-base font-light my-1">{User.resource || "No Resource Found"}</p>
            </div>

            <div className="text-white px-2 mt-3">
              <p className=" text-lg font-semibold">New Skill Target  : </p>
              <p className="text-base font-light my-1">{User.skills || "No Target Skills Found"}</p>
            </div>
          </div>
        </div>
      )}
